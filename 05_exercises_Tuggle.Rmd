...
expeditions_graph <- expeditions %>% 
  ...
  labs(title = "How has the number of days it takes to climb the peaks changed over time?",
...
lower_covid19 <- covid19%>% 
  group_by(state) %>% 
  #top_n(n = 1, wt = state) %>% 
  mutate(state = str_to_lower(state))

covid19_pop <- lower_covid19 %>% 
  left_join(census_pop_est_2018,
            by = c("state")) %>% 
  mutate(day = wday(date, label = TRUE),
         cases_per_10000 = (cases/est_pop_2018)*10000) %>% 
  filter(day == "Fri")
...
covid19_pop %>% 
  ...
  labs(title = "State COVID-19 Cases Over Time",
